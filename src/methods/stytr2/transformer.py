...
Vendored from https://github.com/diyiiyiii/StyTR-2/blob/main/models/transformer.py
with two changes:

* The original module set ``CUDA_VISIBLE_DEVICES`` and pinned ``cuda:2`` at
  import time. Both are stripped — device placement is the caller's concern.
* The unused imports (``function.normal``, ``function.normal_style``, ``os``)
  and the unused ``build_transformer`` factory are removed.
"""
import copy
...
class Transformer(nn.Module):
    ...
    def forward(self, style, mask, content, pos_embed_c, pos_embed_s):
        ...
        # HWxNxC -> NxCxHxW. The original code assumes a square feature map
        # (H = sqrt(N)); we preserve that assumption since the model was
        # trained with square inputs.
        N, B, C = hs.shape
        H = int(np.sqrt(N))
        hs = hs.permute(1, 2, 0)
        hs = hs.view(B, C, -1, H)
